...
关于模块和顶层的使用，有一些实用技巧值得掌握，以便更有效地将两者结合使用。

## 加载编译好的模块

编译 OCaml 文件会生成一个与该文件同名的模块，但首字母大写。这些编译好的模块可以通过 `#load` 加载到顶层中使用。

例如，假设你创建一个名为 `mods.ml` 的文件，并在其中输入以下代码：
...
注意，代码周围并没有 `module Mods = struct ... end`。这些代码可以说位于文件的最顶层。

然后假设你执行 `ocamlc mods.ml` 来编译它。新创建的文件之一是 `mods.cmo`：这是一个编译后的模块对象文件（<u>c</u>ompiled <u>m</u>odule <u>o</u>bject），也称为字节码文件。

你可以使用以下指令使该字节码在顶层中可用。回想一下，指令前面需要加 `#` 字符，它不是提示符的一部分。
...
如果你正在对某个特定模块进行大量测试，每次启动 utop 时都要输入指令会很烦人。你希望顶层在启动时就包含一些代码，这样就不必反复输入这些代码。

解决方案是在工作目录中创建一个名为 `.ocamlinit` 的文件。注意，文件名前面的 `.` 是必需的，它使该文件成为一个[隐藏文件][hidden]，除非明确请求（如 `ls -a`），否则不会出现在目录列表中。`.ocamlinit` 中的所有内容在 utop 启动时都会被处理。
