...
用顶层来测试函数只适合非常小的程序。较大的程序需要包含许多”单元测试”的”测试套件”，并且每次更新代码库时都能重新运行。单元测试是针对程序中一小部分功能（如单个函数）的测试。

现在我们已经掌握了足够多的 OCaml 特性，可以来学习如何使用 OUnit 这个库了。OUnit 是一个单元测试框架，类似于 Java 中的 JUnit、Haskell 中的 HUnit 等。使用 OUnit 的基本工作流程如下：

* 在文件 `f.ml` 中编写函数。该文件中可能还有很多其他函数。

* 在单独的文件 `test.ml` 中编写该函数的单元测试。具体的文件名其实并不重要。
...
以下示例展示了如何创建 OUnit 测试套件。示例中的一些内容乍看可能有些神秘；这些内容将在下一节中讨论。

创建一个新目录。在该目录中，创建一个名为 `sum.ml` 的文件，将以下代码写入其中：
```ocaml
...
根据你的编辑器和配置，你现在可能会看到一些关于 OUnit2 和 Sum 的”未绑定模块”错误。不用担心，代码实际上是正确的。我们只需要配置 Dune，告诉它链接 OUnit。创建一个 `dune` 文件，写入以下内容：
...
回到你的编辑器，做任何能让它重新加载 `test.ml` 的操作。你可以关闭再重新打开窗口，或者对文件做一些小改动（比如添加一个空格再删除）。现在错误应该全部消失了。
...
FFF
```
告诉我们 OUnit 运行了三个测试用例，并且全部三个都失败了（<u>f</u>ailed）。

下一个值得关注的行是：
```
Error: test suite for sum:2:two_elements.
```
它告诉我们，在名为 `test suite for sum` 的测试套件中，索引为 2、名为 `two_elements` 的测试用例失败了。该测试用例的其余输出不是特别有用，我们暂时忽略它。
...
对于比整数更复杂的输出类型，你需要自己编写函数传递给 `printer`。这类似于在 Java 中编写 `toString()` 方法：对于你自己定义的复杂类型，语言不知道如何将它们呈现为字符串，你必须提供相应的代码。
