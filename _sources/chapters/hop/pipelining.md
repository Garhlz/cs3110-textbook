...
与原始尾递归版本相比，所有这些方案的缺点是浪费空间——线性而非常数空间——并且消耗更多的时间（乘上常数因子）。正如编程中经常出现的情况，这是代码清晰度与效率之间的权衡。

注意，低效率*不是*来自管道操作符本身，而是来自必须构建那些不必要的中间列表。所以不要认为管道本质上是不好的。事实上，它非常有用。当我们进入关于模块的章节后，会经常用它来处理那里研究的数据结构。
