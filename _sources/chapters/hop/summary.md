...
本章是本书中最重要的章节之一。它没有介绍任何新的语言特性。相反，我们学习了如何以可能新颖、令人惊讶或具有挑战性的方式来使用已有特性。高阶编程和抽象原则这两个思想将帮助你成为更好的程序员，不仅是 OCaml，也包括其他语言。当然，不同语言对这些思想的支持程度确实不同，有些语言对编写高阶代码的支持要少得多——这也是我们在这门课中使用 OCaml 的原因之一。

`map`、`filter`、`fold` 等函数已被广泛认可为构建计算的优秀方式。部分原因是它们将数据结构上的*迭代*从对每个元素执行的*计算*中分离了出来。Python、Ruby 和 Java 8 等语言现在都支持这种迭代方式。
