...
通过 `bind` 或其他语法向 Promise 注册回调时，回调会加入与该 Promise 一同保存的列表。Promise 最终得到履行后，Lwt 的*解析循环*会运行其中的所有回调，但不保证执行顺序。也就是说，顺序具有非确定性；若顺序很重要，程序员必须使用 `bind`、`join` 等组合运算符明确约束。如果 Promise 永远得不到履行或拒绝，注册的回调也永远不会运行。

```{note}
Lwt 还允许注册在 Promise 被拒绝后运行的函数，`Lwt.catch` 和 `try%lwt` 就用于此目的，分别与 `Lwt.bind` 和 `let%lwt` 相对应。
```

再次强调，并发真正来自操作系统。操作系统层面可能同时进行许多异步 I/O 操作；但在 OCaml 层面，解析循环仍是顺序的，一次只运行一个回调。

解析循环不会主动中断回调。因此，只要某个回调陷入无限循环，其余回调就再也无法运行。正因如此，Lwt 属于协作式并发，而不是抢占式并发。
...
Lwt 对 `bind` 的实现基本遵循我们刚才采用的算法。注意，`bind` 本身并不创造并发；正如前面所说，真正提供并发能力的是操作系统。

```{warning}
当输入 Promise 已经履行，而回调又引发异常时，Lwt 的处理与这里不同：`bind` 会直接抛出异常，而不是返回一个携带该异常的已拒绝 Promise。这是一个[已知的][lwt-bind-bug] `bind` 规范违例，但出于向后兼容考虑不会修复。
```
