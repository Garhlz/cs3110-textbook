...
模块系统的主要关注点之一是提供*封装*（encapsulation）：把实现的信息隐藏在接口后面。OCaml 的模块系统通过我们已经见过的特性使封装成为可能：模块类型标注所带来的*不透明性*（opacity）。不透明性的一种特殊用途是声明*抽象类型*。我们将在本节中研究这两个概念。

## 不透明性

实现模块时，有时你可能会使用一些不想暴露给模块客户端的辅助函数。例如，假设你在实现一个提供尾递归阶乘函数的数学模块：
...
你希望 `fact` 对 `Math` 的客户端可见，但希望 `fact_aux` 保持隐藏。然而在上面的代码中，你可以看到 `fact_aux` 出现在编译器为 `Math` 推断的签名中。隐藏它的一种方法是将 `fact_aux` 嵌套进去：
...
查看签名，注意 `fact_aux` 是如何消失的。但这种嵌套让 `fact` 略显难读，也意味着 `fact_aux` 不能被 `Math` 内部的任何其他函数使用。在当前情况下这可能没问题——`Math` 中可能没有其他需要 `fact_aux` 的函数。但如果有的话，我们就无法嵌套 `fact_aux`。

因此，还有一种方法既能对 `Math` 的客户端隐藏 `fact_aux`，又能让 `Math` 的实现者继续使用它：使用一个模块类型，只公开客户端需要看到的那些名称：
