...
*函数式数据结构*是不依赖可变性的数据结构。它既可以用函数式语言构建，也可以用命令式语言构建。例如，在 Java 中定义一个以 `final` 修饰字段的不可变 `Node` 类，就能构造出与 OCaml `list` 类型相当的数据结构。

函数式数据结构具有*持久性*（persistent）：对它执行“更新”操作时，现有版本不会被改写，而是会产生一个新版本，新旧版本可以并存并继续访问。优秀的语言实现会在两个版本之间*共享*没有发生变化的部分，只*复制*确实改变的部分，从而保留旧版本。与之相对的是*临时*（ephemeral）数据结构：更新会破坏原有版本，因此任一时刻只有一个版本存在。这两类数据结构都能用函数式语言或命令式语言构建。

## 列表

OCaml 内置的单链 `list` 是函数式数据结构；从此前用代数数据类型实现列表的过程便可看出这一点。它同时也是持久数据结构，下面的例子可以证明：
...
取得 `lst` 的尾部并不会改变原列表，`lst` 和 `lst'` 可以互不影响地并存。
