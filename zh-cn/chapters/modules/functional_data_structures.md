...
"函数式数据结构"是一种不利用可变性的数据结构。函数式数据结构可以用函数式语言构建，也可以用命令式语言构建。例如，在 Java 中你可以通过创建字段被 `final` 关键字设为不可变的 `Node` 类，来构建等价于 OCaml `list` 类型的数据结构。

函数式数据结构具有*持久性*（persistent）：对数据结构执行某种操作来"更新"它，并不会改变现有版本的数据结构，而是产生一个新版本。新旧两个版本并存，且仍然可以访问。好的语言实现会确保数据结构中未被操作改变的部分在旧版本和新版本之间被*共享*。任何发生变化的部分则会被*复制*，以便旧版本得以保留。与持久数据结构相对的是*短暂*（ephemeral）数据结构：更改具有破坏性，因此任何时候都只有一个版本。持久数据结构和短暂数据结构都可以用函数式语言和命令式语言构建。
